...
class DataAnggota extends Controller
{
    ...
    public function data_admin()
    {
        ...
        return view('admin.data_anggota.admin', compact('admins'));
    }

    public function data_anggota()
    {
        $users = User::where('role', 'user')->get();
        return view('admin.data_anggota.user', compact('users'));
    }
    ...
    public function submit_anggota(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return redirect()->route('admin.data_anggota')->with('status', 'danger')->with('message', "Gagal Menambah Anggota, " . $errors);
        }
        ...
        if ($request->photo) {
            ...
            try {
                ...
                return redirect()->route('admin.data_anggota')->with(
                    'status',
                    'success'
                )->with('message', 'Berhasil Menambah Anggota');
            } catch (Exception $e) {
                return redirect()->route('admin.data_anggota')->with('status', 'danger')->with('message', "Gagal Menambah Anggota" . $e);
            }
        }
        $user = User::create([
            'kode' => $generate_code,
    ...
    public function submit_admin(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return redirect()->route('admin.data_admin')->with('status', 'danger')->with('message', "Gagal Menambah Admin, " . $errors);
        }
        ...
        if ($request->photo) {
            ...
            try {
                ...
                return redirect()->route('admin.data_admin')->with(
                    'status',
                    'success'
        ...
        if ($user && $user2) {
            return redirect()->route('admin.data_admin')->with("status", "success")->with(
                'message',
