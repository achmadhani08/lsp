                ...
                <div class="modal-content">
                    ...
                    <form action="{{ route('admin.submit_pemberitahuan') }}" method="POST" class="form-group">
                        ...
                        <div class="modal-body">
                            <div class="card-body">
                                <div class="mb-3">
                                    <textarea style="height: 100px" placeholder="Isi Pemberitahuan" type="text" name="isi" class="form-control"
                                        required></textarea>
                                </div>
                                <div class="d-flex justify-content-center">
                                    <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
                                        <input type="radio" class="btn-check" value="aktif" name="status" id="btnradio1"
                                            autocomplete="off" checked>
                                        <label class="btn btn-outline-primary" for="btnradio1">General</label>

                                        <input type="radio" class="btn-check" value="admin" name="status" id="btnradio2"
                                            autocomplete="off">
                                        <label class="btn btn-outline-light" for="btnradio2">Only Admin</label>

                                        <input type="radio" class="btn-check" value="user" name="status" id="btnradio3"
                                            autocomplete="off">
                    ...
                    <div class="modal-content">
                        ...
                        <form action="{{ route('admin.activate_notif', $pemberitahuan->id) }}" method="post"
                            ...
                            <div class="modal-body">
                                <div class="card-body">
                                    <div class="mb-3">
                                        <textarea style="height: 100px" placeholder="Isi Pemberitahuan" type="text" name="isi"
                                            value="{{ $pemberitahuan->isi }}" class="form-control" required>{{ $pemberitahuan->isi }}</textarea>
                                    </div>
                                    <div class="d-flex justify-content-center">
                                        <div class="btn-group" role="group"
                                            aria-label="Basic radio toggle button group">
                                            <input type="radio" class="btn-check" value="aktif" name="status"
                                                id="btnradio6{{ $pemberitahuan->id }}" autocomplete="off" checked>
                                            <label class="btn btn-outline-primary"
                                                for="btnradio6{{ $pemberitahuan->id }}">General</label>

                                            <input type="radio" class="btn-check" value="admin" name="status"
                                                id="btnradio7{{ $pemberitahuan->id }}" autocomplete="off">
                                            <label class="btn btn-outline-light"
                                                for="btnradio7{{ $pemberitahuan->id }}">Only Admin</label>

                                            <input type="radio" class="btn-check" value="user" name="status"
                                                id="btnradio8{{ $pemberitahuan->id }}" autocomplete="off">
                                            <label class="btn btn-outline-success"
                                                for="btnradio8{{ $pemberitahuan->id }}">Only User</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
